...
demographics = readtable('data/Demographics.xls');
measurements = readtable('data/Measurements.xls');
caesar = join(demographics, measurements);
clear demographics measurements

%% remove subjects that are missing corresponding 3D scan data
todel = [150; 264; 268; 307; 550; 1303; 2178; 2236];
caesar(todel,:) = [];
clear todel

%% read autoencoder body shape parameters
load('data/bodyshape.mat');
%load('data/bodyshape_backup_180717_normalized.mat');
caesar = [caesar array2table(double(AE))];
% caesar = [caesar array2table(double(AE(:,[2,1])))];
clear AE
